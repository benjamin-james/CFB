...
function fetch_url(client::CfbdClient, url::String; retries::Int=5, wait_time::Int=2)
    ...
    try
        ...
        if e isa HTTP.ExceptionRequest.StatusError
            ...
            if status == 429 && retries > 0
                println("Received 429 Too Many Requests. Retrying in $wait_time seconds ")
                sleep(wait_time)
                return fetch_url(client, url; retries=retries-1, wait_time=wait_time*2)
            else
                println("HTTP Error $status: $(e.message)")
                rethrow(e)
            end
        else
...
function fill_results(client::CfbdClient; since::Union{DateTime,Nothing} = nothing)
    table_names = [table_schema.name for table_schema in SQLite.tables(client.db)]
    if since !== nothing
        epoch = datetime2unix(since)
    elseif "Game" in table_names && "Calendar" in table_names && "Play" in table_names && "Drive" in table_names
        ef = DataFrame(SQLite.DBInterface.execute(client.db, """
SELECT * FROM Game
...
JOIN Play ON Play.gameId = Game.id
    ...
    for season in range(Dates.year(since), Dates.year(now()))
        println(season)
        fetch_calendar(client, season)
        fetch_games(client, season)
        fetch_drives(client, season)
        cal = DataFrame(SQLite.DBInterface.execute(client.db, "SELECT * FROM Calendar WHERE Calendar.season == $(season) AND Calendar.startDateUnix >= $(epoch);"))
        for row in eachrow(cal)
            fetch_plays(client, row.season, row.week, seasonType=row.seasonType)
        end
    end
end
function fetch_calendar(client::CfbdClient, season::Int)
    results = fetch_url(client, "calendar?year=$(season)")
    if results === nothing
        return nothing
    end
...
CREATE TABLE IF NOT EXISTS Calendar (
    ...
    stmt = SQLite.Stmt(client.db, "INSERT OR REPLACE INTO Calendar (season, week, seasonType, startDate, endDate, startDateUnix, endDateUnix) VALUES (?, ?, ?, ?, ?, ?, ?)")
    for item in results
...
function fetch_drives(client::CfbdClient, season::Int; week::Union{Int, Nothing}=nothing, seasonType::Union{String, Nothing}=nothing)
    path = "drives?year=$(season)"
    if week !== nothing
        path = "$(path)&week=$(week)"
    end
...
CREATE TABLE IF NOT EXISTS Drive (
    ...
    startYardsToGoal INTEGER NOT NULL,
    startTime_minutes INTEGER,
    startTime_seconds INTEGER,
    endPeriod INTEGER NOT NULL,
    endYardline INTEGER NOT NULL,
    endYardsToGoal INTEGER NOT NULL,
    endTime_minutes INTEGER,
    endTime_seconds INTEGER,
    plays INTEGER NOT NULL CHECK (plays >= 0),
    yards INTEGER NOT NULL,
    driveResult TEXT NOT NULL,
    isHomeOffense INTEGER NOT NULL CHECK (isHomeOffense IN (0, 1)),
    startOffenseScore INTEGER NOT NULL,
    endOffenseScore INTEGER NOT NULL,
    startDefenseScore INTEGER NOT NULL,
    endDefenseScore INTEGER NOT NULL
        );""")
    stmt = SQLite.Stmt(client.db, "INSERT OR REPLACE INTO Drive (id, gameId, offense, offenseConference, defense, defenseConference, driveNumber, scoring, startPeriod, startYardline, startYardsToGoal, startTime_minutes, startTime_seconds, endPeriod, endYardline, endYardsToGoal, endTime_minutes, endTime_seconds, plays, yards, driveResult, isHomeOffense, startOffenseScore, endOffenseScore, startDefenseScore, endDefenseScore) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
    for item in results
        SQLite.execute(stmt, (item.id, item.gameId, item.offense, item.offenseConference, item.defense, item.defenseConference, item.driveNumber, item.scoring, item.startPeriod, item.startYardline, item.startYardsToGoal, item.startTime.minutes, item.startTime.seconds, item.endPeriod, item.endYardline, item.endYardsToGoal, item.endTime.minutes, item.endTime.seconds, item.plays, item.yards, item.driveResult, item.isHomeOffense, item.startOffenseScore, item.endOffenseScore, item.startDefenseScore, item.endDefenseScore))
    end
    SQLite.execute(client.db, "COMMIT TRANSACTION")
end

function fetch_games(client::CfbdClient, season::Int)
    results = fetch_url(client, "games?year=$(season)")
    if results === nothing
        return nothing
    end
    SQLite.execute(client.db, "BEGIN TRANSACTION")
